        ...
        <template v-else>
            ...
            >
                ...
                <figure>
                    ...
                    <figcaption>
                        <p v-if="!hideBrandName" class="text_brand">
                            {{ goods.brandName }}
                        </p>
                        <p class="text_product">
                            <span v-if="goods.headline !== undefined && goods.headline !== null" class="text_foreword">[{{ goods.headline }}]</span>
                        ...
                        <div class="mm_tag-list">
                            <b v-if="approachSoldout" class="mm_tag T=square T=product-status">품절임박</b>
                            <b v-for="badge in displayedBadges" :key="badge" class="mm_tag T=square T=dark">{{ badge }}</b>
                            <b v-if="!approachSoldout && displayedBadges.length < 1" class="mm_tag T=square T=dark" style="opacity: 0;">-</b>
                        </div>
...
export default defineComponent({
    ...
    setup(props, { attrs }) {
        ...
        const displayedBadges = computed(() => {
            if (!goods.value.badges) {
                return []
            }

            if (approachSoldout.value) {
                return goods.value.badges.slice(0,1);
            }

            return goods.value.badges
        })
