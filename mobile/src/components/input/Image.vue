<template>
    <div class="mm_form-multiple">
        <ul :class="['mm_flex', isTEqual ? 'T=equal' : '']">
            <li v-for="index in max" :key="index">
                <div :ref="setImageDivElements" class="mm_form-image">
                    <button type="button" class="btn_remove-file" @click="removeImage(index - 1)">
                        <i class="ico_image-remove" /><b class="mm_ir-blind">파일삭제</b>
                    </button>
                    <div class="mm_form-image-preview" />
                    <label>
                        <input :ref="setImageInputElements" type="file" :accept="accept" @change="selectImage">
