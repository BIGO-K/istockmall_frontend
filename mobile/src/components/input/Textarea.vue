...
<div 
    ...
    :class="[
            { 'S=text-on' : modelValue.length > 0 },
            { 'S=byte': maxByte > 0 && !isChanged },
            { 'S=text-changed': isChanged },
            { 'S=byte': maxByte > 0 },
        ]"
    >
    <button 
    ...
    <label>
        ...
        />
        <i class="bg_text"></i>
        <span class="text_placeholder" v-html="placeHolderText"></span>
    </label>
    <b v-if="maxByte > 0" class="text_byte"><strong>{{ modelValue.length }}</strong>/{{ maxByte }}</b>
</div>
</template>
<script lang="ts">
import { defineComponent, computed, toRefs, ref, onMounted, PropType  } from 'vue';
