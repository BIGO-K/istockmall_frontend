                        ...
                        <div class="m_popup-review-detail">
                            ...
                            >
                                <template #item="{ item: review }">
                                    <div v-if="review !== undefined" class="mm_review">
...
const reviewId = Number(reviewModalInfo.value.reviewId);
const firstReviewId = Number(reviewModalInfo.value.firstReviewId);
const lastReviewId = Number(reviewModalInfo.value.lastReviewId);
const totalCount = Number(reviewModalInfo.value.totalCount);
...
const maxItemCount = computed(() => {
    ...
    return  totalCount > 3 ? 4 : totalCount;
});
...
async function fetchSideReviewInfo() {
    ...
    try {
        // 이전(=>), 다음(<=) 리뷰의 캐러셀 index
        const nextIndex = currentIndex.value === maxItemCount.value - 1 ? 0 : currentIndex.value + 1;
        const prevIndex = currentIndex.value === 0 ? maxItemCount.value - 1 : currentIndex.value - 1;
...
async function fetchReviewInfo(goodsId: number, reviewId: number, index: number) {
    let review = totalReviews.value.find(review => review.reviewDetail.id === reviewId);

    // 조회 및 전체 조회 목록에 추가
    if (review === undefined) {
        try {
            review = await goodsRepository.bestDetailReview(goodsId, reviewId);
