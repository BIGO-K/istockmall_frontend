        ...
        >
            <template #count>
                <a 
                    ...
                    @click.capture="setBannersPopup('b_c=w6Xh8_s1_pager', '기획전 전체보기', (block.bannerPartition?.banners || []))"
                >
                    <strong class="text_carousel-index" /><i>/</i><span class="text_carousel-total" /><i class="uico_plus" />
                </a>
            </template>
            <template #item="{ item }">
                <MMBanner :banner="item" />
            </template>
        </MMCarousel>
    </MMBlock>
</template>
...
<style>
@import '@publish/css/block/basic/b_c=w6Xh8_s1_pager.css';
</style>
