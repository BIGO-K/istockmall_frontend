<template>
    <MMBlock
        :block-name="'pers_tp_sn=w6Xh21_s6_refresh'"
        :required="{
            ...
            <div class="mui_tab" data-tab>
                    ...
                    >
                        <template #item="{ item, index }">
                            ...
                            <div v-if="showFitPopup && index === 0 && item.id == selectedTabId" class="m_pers-size-pop">
                                ...
                                <MMLink class="btn_write" :to="{ name: 'MypageMySize' }">
                                    <b>입력하러가기</b>
                                </MMLink>
                ...
                <div v-for="tab in block.tabs" :key="tab.id" class="mui_tab-item" :class="tab.id === selectedTabId ? 'S=tab-on' : ''">
                    <div class="mui_product-list">
                        <ul>
                            <li 
                                v-for="goods in (tab.goodsGroup?.goodsList || []).slice(0, (tab.goodsGroup?.perPage || defaultPerPage))"
                                :key="goods.id"
                            >
                                <MMBlockGoods 
                                    :goods="goods" 
...
<style>
    @import "@publish/css/block/feature/pers_tp_sn=w6Xh21_s6_refresh.css";
</style>