...
const { mySizeInfo: mySizeForm } = toRefs(props);
mySizeForm.value.informationAgree = false;
...
async function updateMySize() {
    const validator = makeValidator(mySizeForm.value)
        .setRules({
            'gender(성별)': ['required'],
            'fitId(My 핏)': [`in:${(props.fitList.map(fit => fit.id)).join()}`],
            'shoesCategoryCode(신발 카테고리)': [`in:${(props.shoesCategories.map(category => category.code)).join()}`],
            'shoesSizeId(신발 사이즈)': [`in:${(props.shoesSizes.map(size => size.id)).join()}`],
