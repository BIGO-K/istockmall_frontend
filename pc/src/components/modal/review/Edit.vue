<template>
    ...
    <div class="m_modal-myreview-product">
        ...
        <div class="m...product-point S=point-on">
            ...
            <div
                class="mm_dropdown"
                :class="{'S=on': isShowReviewPointInfo }"
                data-dropdown
                @click="isShowReviewPointInfo = !isShowReviewPointInfo"
            >
                <button
                    type="button"
                    class="btn_dropdown"
                    :title="isShowReviewPointInfo ? '펼쳐보기' : ''"
                >
                ...
                <div
                    ...
                    :style="isShowReviewPointInfo ? { height: 'auto' } : {}"
                >
                ...
                <a>
                    <figure>
                        <div class="mm_image-scale">
                            <i
                                v-lazyload="{ src: writtenReview.orderItem.goods.thumbnailUrl }"
                                class="mm_bg-cover image_product"
                            />
                        </div>
                        <figcaption>
                            <p class="text_product">{{ writtenReview.orderItem.goods.name }}</p>
                            <p class="text_option">{{ writtenReview.orderItem.goods.optionName }} / {{ writtenReview.orderItem.goods.ea }}개</p>
                        </figcaption>
                    </figure>
                </a>
            </div>
    ...
    <div class="m_modal-myreview-value">
            ...
            <div class="m...value-starscore">
                ...
                <ul>
                    <li 
                        v-for="score in 5"
                        :key="score"
                        :class="{'S=starscore-on': review.rate >= score}"
                    >
                        <button
                            type="button"
                            :title="review.rate == score ? '선택됨' : ''"
                            @click="review.rate = score"
                        >
                            <i
                                class="ico_star"
                                title="별점"
        ...
        <!--// 별점 -->
        <!-- 리뷰선택 항목 -->
        <template v-if="review.templates.length > 0">
            <section v-for="template, index in review.templates" :key="template.id">
                <h5><b>{{ MMFilters.applyZosa(`${template.title}(은/는)`) }} 어떤가요?</b></h5>
                <div class="mm_radio-list">
                    <ul class="mm_flex T=equal">
                        <li
                            v-for="detail in template.details"
                            :key="detail.id"
                        >
                            <!-- :value="`${template.id}_${detail.id}`" -->
                            <MMRadio 
                                v-model="templateIds[index]"
    ...
    <div class="m_modal-myreview-info">
        <template v-if="hasPersonalization">
            ...
            <div class="mm_form_mix-linked">
                ...
                <MMSelect v-model="review.shoesSize">
                    <option value="">
                        발 사이즈
                    </option>
                    <option
        ...
        <MMImage
            v-model="images"
            :image-urls="writtenReview.imageUrls"
            accept="image/*"
        />
        <div class="mm_note">
            <ul>
...
import { EditableReview, Review } from '$/@type/member/review';
...
// 등급별 리뷰 포인트 안내 영역 노출
const isShowReviewPointInfo = ref<boolean>(false);
// 포인트 명칭
...
const templateIds = ref<string[]>(
    review.value.templates.map((template) => {
        return `${template.id}_${review.value.selectedTemplates.find(selected => selected.templateId === template.id)?.detailId || ''}`; 
    })
);
...
async function editReview() {
    ...
    try {
        ...
        formData.append('rate', review.value.rate === 0 ? '' : `${review.value.rate}`);
        formData.append('contents', review.value.contents);
        formData.append('height', `${review.value.height || ''}`);
        formData.append('weight', `${review.value.weight || ''}`);
        formData.append('shoes_size', `${review.value.shoesSize || ''}`);
        ...
        const validator = makeValidator(formData)
            ...
            .registTester('mimes', function (param: string, extraValue:string, data:Object) {
                const file = getTargetValue(data, param)
        ...
        await validator.run();
        await reviewRepository.update(review.value.id, formData);
