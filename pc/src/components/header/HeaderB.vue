<template>
    <div
        ref="headerElement"
        :class="['mm_header T=hb', { 'S=header-sticky': isHeaderSticky, 'T=text-white': isFontWhite }]"
    >
        <div class="mm_inner">
            <div class="mm_header-inner">
                <h1>
                    <router-link
                        :to="{ name: 'Main' }"
                ...
                >
                    ...
                    <div class="mm_header-search-form">
                        <div class="mm_form-text" :class="searchKeyword ? 'S=on' : ''">
                            ...
                            <label>
                                ...
                                />
                                <span v-else class="text_placeholder">검색어를 입력하세요</span>
                            </label>
                        </div>
                    ...
                    >
                        ...
                        <template v-if="autoCompletes.categories.length + autoCompletes.keywords.length > 0">
                                ...
                                <ul>
                                    <li v-for="(brand, index) in autoCompletes.brands" :key="brand.id">
                                        <MMLink
                                            ...
                                            @click="() => {
                                                searchKeywordEl!.value = brand.korName;
                                                closeSearchWord();
                                            }"
                                        >
                                            <figure>
                                                ...
                                                <figcaption>
                                                    <b v-html="MMFilters.wrap(brand.korName, searchKeyword, `<strong class='mm_text-secondary'>`, '</strong>')" />
                            ...
                            <ul v-if="autoCompletes.categories.length > 0">
                                <li v-for="(category, index) in autoCompletes.categories" :key="index">
                                    <MMLink
                                        ...
                                        @mouseleave="autoCompletes.focusOnIndex = -1"
                                        @click="() => {
                                            searchKeywordEl!.value = category.fullPath;
                                            closeSearchWord();
                                        }"
                                    >
                            ...
                            <ul v-if="autoCompletes.keywords.length > 0">
                                <li v-for="(keyword, index) in autoCompletes.keywords" :key="index">
                                    ...
                                    >
                                        <b v-html="MMFilters.wrap(keyword, searchKeyword, `<strong class='mm_text-secondary'>`, '</strong>')" />
                                    </MMLink>
                                </li>
                            </ul>
                        </template>
                ...
                <div class="mm_header-quick">
                    <ul>
                        ...
                        <li>
                            <MMLink :to="{ name: 'Cart' }">
                                <i class="ico_pushcart" /><b class="mm_ir-blind">장바구니</b><strong class="text_count">{{ inCartGoodsCount }}</strong>
                            </MMLink>
                        </li>
                        <li>
                            <MMLink :to="{ name: 'MypageWishGoods'}">
                                <i class="ico_like" /><b class="mm_ir-blind">찜한 상품</b>
                            </MMLink>
                        </li>
                        <li>
                            <div
            ...
            <div class="mm_gnb">
                ...
                <!--
                        ...
                        .mm_gnb...depth 안 section 태그에 mousehover 시 'S=gnb-on' 클래스가 추가됩니다.
                    -->
                <div
                    
                    :class="['mm_gnb-cate', categoryMenuClassSwitchOn ? switchOnClassName : '']"
                    @mouseleave="handleMouseLeave"
                >
                    <button
                        type="button"
                        class="mm_switch btn_catemenu"
                        ...
                        title="펼쳐보기"
                        @click="categoryMenuClassSwitchOn = !categoryMenuClassSwitchOn"
                    >
                        <i class="ico_menu" /><b>카테고리</b>
                    </button>
                    <div
                        v-show="categoryMenuClassSwitchOn"
                        class="mm_gnb-cate-item"
                    >
                        <nav ref="categoryNavContainer" :style="`width:${categoryWidthDefault}px`">
                            <ul>
                                <li
                                    v-for="category in categories"
                                    :key="category.code"
                                    @mouseenter="handleCategoryDepth1MouseEnter"
                                    @mouseleave="handleCategoryDepth1MouseLeave"
                                >
                                    <MMLink
                                        :class="{ 'S=catemenu-on' : selectedDepth1CategoryCode == category.code }"
                                        :to="{
                                            name : 'ExhibitCategoryIndex',
                                            params: { id: category.code }
                                        }"
                                        @mouseover="handleDepth1CategoryMouseOver($event, category.code)"
                                    >
                                        <i
                                    ...
                                    <div class="mm_gnb...depth">
                                        <div class="mm_scroller">
                                            ...
                                            >
                                                <section
                                                    :class="{ 'S=catemenu-on' : selectedDepth2CategoryCode == depth2Category.code }"
                                                    @mouseenter="handleCategoryDepth2MouseEnter"
                                                    @mouseleave="handleCategoryDepth2MouseLeave"
                                                >
                                                    <h4>
                                                        <MMLink
                                                            :to="{
                                                            ...
                                                            @mouseover="handleDepth2CategoryMouseOver($event, depth2Category.code)"
                                                        >
                                                    ...
                                                    <ul>
                                                        <li>
                                                            <MMLink
                                                                :class="[categoryMenuClassSwitchOn ? switchOnClassName : '']"
                                                                :to="{
                                                                    name : 'GoodsCategoryIndex',
                                                                    params: { id: depth2Category.code }
                                                                }"
...
const { basicSearchKeyword } = toRefs(props)
const headerElement = ref<HTMLElement|null>(null)

useEventListener(window, 'scroll', windowScrollEventHandler);
const {
    route,
    router,
    mmon,
...
const searchPlaceholder = computed(() => {
    return {
        text: basicSearchKeyword.value?.text,
        customLink: basicSearchKeyword.value?.customLink,
    }
})
...
// 카테고리 nav
const categoryNavContainer = ref<HTMLElement|null>(null)
// 선택된 카테고리 코드
const selectedDepth1CategoryCode = ref<string>('')
const selectedDepth2CategoryCode = ref<string>('')
// 1뎁스,2뎁스 카테고리 기본값
const categoryWidthDefault = 218;
const depth1CategoryWidth = ref<number>(categoryWidthDefault)
const depth2CategoryWidth = ref<number>(categoryWidthDefault);
function handleCategoryDepth1MouseEnter(event: MouseEvent) {
    if (depth1CategoryWidth.value < categoryWidthDefault) {
        depth1CategoryWidth.value = categoryWidthDefault;
    }
    const element = event.target as HTMLElement
    // mm_gnb...depth div 기본 너비 218 (아무것도 없는 경우 대비!)
    depth2CategoryWidth.value = (element.children[1] as HTMLElement)?.offsetWidth || categoryWidthDefault;
    const nowWidth = depth1CategoryWidth.value + (selectedDepth1CategoryCode.value !== '' ? depth2CategoryWidth.value : categoryWidthDefault)

    gsap.to(categoryNavContainer.value, { width: nowWidth, duration: 0.4, ease: 'sine.inOut', overwrite: true })
    if (categoryNavContainer.value) {
        const categoryUlElement = categoryNavContainer.value.querySelector('ul');
        if (categoryUlElement) {
            const categoryUlElementChild = categoryUlElement.querySelector('li');

            const cateLimit = categoryUlElementChild?.offsetHeight || 0;
            const cateMoveHeight = categoryNavContainer.value.offsetHeight - categoryUlElement?.offsetHeight
            
            if (cateMoveHeight > 0) {
                return;
            }

            let _ratioY = (event.clientY - categoryNavContainer.value.getBoundingClientRect().top - cateLimit) / (categoryNavContainer.value.offsetHeight - cateLimit * 2);
            _ratioY = Math.max(0, Math.min(_ratioY, 1));

            gsap.to(categoryUlElement, { 'margin-top':  cateMoveHeight * _ratioY, duration: 0.4, ease: 'quad.out' });
        }
    }
}

function handleCategoryDepth1MouseLeave() {
    gsap.to(categoryNavContainer.value, {
        width: depth1CategoryWidth.value,
        duration: 0.2,
        delay: 0.1,
        ease: 'sine.inOut',
        overwrite: true,
    })
    selectedDepth1CategoryCode.value = ''
}

function handleCategoryDepth2MouseEnter(event: MouseEvent) {
    const element = event.target as HTMLElement

    const depth3CategoryWidth = selectedDepth2CategoryCode.value !== '' ? (element.children[1] as HTMLElement)?.offsetWidth || categoryWidthDefault : categoryWidthDefault;
    
    // mm_gnb...depth div 기본 너비 (아무것도 없는 경우 대비!)
    const nowWidth = depth1CategoryWidth.value + depth2CategoryWidth.value + depth3CategoryWidth
    gsap.to(categoryNavContainer.value, { width: nowWidth, duration: 0.4, ease: 'sine.inOut', overwrite: true })
}

function handleCategoryDepth2MouseLeave() {
    gsap.to(categoryNavContainer.value, {
        width: depth1CategoryWidth.value + depth2CategoryWidth.value,
        duration: 0.2,
        delay: 0.1,
        ease: 'sine.inOut',
        overwrite: true,
    })
    selectedDepth2CategoryCode.value = ''
}
/**
 * MouseOver 이벤트 핸들러
 *
 * @param event
 * @param depth1CategoryCode
 */
function handleDepth1CategoryMouseOver(event: MouseEvent, depth1CategoryCode: string) {
    selectedDepth1CategoryCode.value = depth1CategoryCode
    depth1CategoryWidth.value = (event.target as HTMLElement)?.offsetWidth
}
 ...
 *
 * @param event
 * @param depth2CategoryCode
 */
function handleDepth2CategoryMouseOver(event: MouseEvent, depth2CategoryCode: string) {
    selectedDepth2CategoryCode.value = depth2CategoryCode
}
 ...
 * @param {Event} event
*/
function openSearchWord(event: Event) {
...
function categoryKeywordWrapper(categoryFullPath: string, depth: number): string {
    ...
    return wrap(categoryDepthSplitText.join(''), searchKeyword.value, '<strong class="mm_text-secondary">', '</strong>')
}
/**
...
function handleSearchKeywordKeydownEvent(event: KeyboardEvent) {
    ...
    case 'ArrowUp':
        ...
        if (autoCompletes.value.focusOnTab == 'category') {
            ...
            // 검색어가 없었던 경우 최근 검색어 있는 경우 해당 검색어로 처리
            autoCompletes.value.focusOnRecentIndex =
        autoCompletes.value.focusOnRecentIndex < 1 ? recentSearchRecords.value.length - 1 : autoCompletes.value.focusOnRecentIndex - 1
        }
...
const showPageMoveAnchor = ref(false)        
const scrollY = computed(() => {
    return headerElement?.value?.offsetHeight || 0;
});
    ...
    if (!ticking.value) {
        window.requestAnimationFrame(() => {
            ...
            if (props.sideBarElement) {
                const wingBanner:HTMLElement|null = props.sideBarElement?.querySelector('.mm_sidebar-wing');
                if (window.scrollY > scrollY.value && !showPageMoveAnchor.value) {
                    showPageMoveAnchor.value = true
                    gsap.to(props.sideBarElement?.querySelector('.mm_sidebar-rside-inner'), { height: 214, duration: 0.3, ease: 'sine.out' })
                    gsap.to(props.sideBarElement?.querySelectorAll('[class*="btn_anchor"]'), {
                        autoAlpha: 1,
                        duration: 0.2,
                        delay: 0.2,
                        ease: 'sine.out',
                ...
                } else if (window.scrollY < scrollY.value && showPageMoveAnchor.value) {
                    showPageMoveAnchor.value = false
                    gsap.to(props.sideBarElement?.querySelector('.mm_sidebar-rside-inner'), { height: 76, duration: 0.3, ease: 'sine.inOut' })
                    gsap.to(props.sideBarElement?.querySelectorAll('[class*="btn_anchor"]'), { autoAlpha: 0, duration: 0.2, ease: 'sine.out' })
                ...
                if (wingBanner) {
                    wingBanner.style.left = props.sideBarElement?.classList.contains('S=sidebar-sm') ? `-${window.scrollX}px`: '';
                }
            }
...
<style>
:root {
    --header_height_base: 148px;
    --header_height_sm: 60px;
}
</style>