<template>
    <div v-show="isShow" class="mm_mui" :class="isFullWidth ? 'T=full' : ''">
        <component
...
const isFullWidth = computed<boolean>(() => {
    ...
    return muiCode.includes('w0')
})

const loadedBlockComponentNames = ref<string[]>([])

const isShow = computed<boolean>(() => {
    return loadedBlockComponentNames.value.length > 0 && !blockComponents.value.every((component) => component === undefined)
});
