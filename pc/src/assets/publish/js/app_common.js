	...
	var base = {
		...
		update: function (__$element) {
			...
			if ($items.length === 1) {
				...
				mm.element.remove([$btnPrev?.parentElement, $count]);
				mm.element.remove($btnPages[0]?.closest('.mm_carousel-pagination'));
				base.set(__$element);
		...
		gsapTo: function (__$element, __direction, __ease, __index) {
			...
			gsap.to(data.__, {
				...
				onComplete: function () {
					...
					//! set이 없을 경우 슬라이더에 들어가는 아이템의 너비가 각각 다를때 아이템 너비에 따라 슬라이더가 동작하지 않는 경우가 발생합니다.
